class KeysController < ApplicationController
  ...
  def create #action for uploading keyfiles and parsing individual records then storing them in db.
    @sample_upload_file = @key.keyfile.to_s.split('/').last
    @key[:upload_file] = @sample_upload_file
    @number_samples_added = 0 #counter for keeping track of samples added to db
    @number_samples_updated = 0
    @number_samples_failed = 0
    @number_duplicate_samples = 0
          ...
          if IDR.find_by(indigo_id: sample_data["INDIGO_ID"]) != nil #checks if an IDR with this INDIGO_ID is already in the database.
            ...
            @existingsample = Sample.find_by(indigo_id: sample_data["INDIGO_ID"])
              ...
              if @idr.save #attempts to save the idr model to the db
                ...
                @batch = Batch.find_by(dataset_id: @current_dataset_id, date_received_from_source: sample_data["Date Samples Received"], samples_sent_to_stanford: sample_data["Date to Stanford"])
                  if @batch == nil
                    @batch = Batch.create(dataset_id: @current_dataset_id, date_received_from_source: sample_data["Date Samples Received"], samples_sent_to_stanford: sample_data["Date to Stanford"])
                  end
        ...
        excel_spreadsheet.drop(1).each do |row| #excel_spreadsheet is an array of arrays. the first array(row) are the header names which are not needed, so the .each iteration starts with the second row
            if IDR.where(indigo_id: row[1]).exists?
              ...
              @idr = IDR.new(sample_source: row[2], disease: row[3], indigo_id: row[1], age_at_sample: row[6],
                ...
                end #close if @idr.save block
            end # close if IDR.where(indigo_id: row[1]).exists? block
          end
        else
          @error_message += "#{sample_upload_file} file format compatible with this application. Please use .csv files"
        end #ends the if @key[:keyfile].split block
      else
        @attempted_duplicate_upload = true
        @error_message += "#{@sample_upload_file} has already been uploaded"
      end #ends the if @key.save block.

    @report = UploadReport.new
    @results = {}
    @results[:file] = @sample_upload_file
    @results[:uploads] = @number_samples_added
    @results[:failed_uploads] = @number_samples_failed
    @results[:upload_type] = "sample"
    @results[:uploaded_by] = current_user.id
    @results[:duplicates] = @number_duplicate_samples
