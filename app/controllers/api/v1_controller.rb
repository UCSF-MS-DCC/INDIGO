class Api::V1Controller < ApplicationController
  ...
  def jsondata #rename this to something like samples
    ...
    samples = Sample.where(sample_phenotypes).select("indigo_id, sample_source, gender, disease, age_of_onset, sample_source_identifier, ethnicity, race, age_at_sample")
  ...
  def rstudiodata  #rename this to something like samplesrstudioformat
    ...
    samples = Sample.where(sample_phenotypes).select("indigo_id, sample_source_identifier, gender, disease, age_of_onset, ethnicity, race, age_at_sample, sample_source")
    ...
    results = { indigo_id: [], sample_source: [], sex: [], disease:[], age_of_onset: [], ethnicity: [], race: [], age_at_sample: [] }
    ...
    samples.each do |s|
      ...
      if s.sample_source != nil
        results[:sample_source].push(s.sample_source)
      else
        results[:sample_source].push("NA")
      end
