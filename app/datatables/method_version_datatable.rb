class MethodVersionDatatable
  delegate :params, :h, to: :@view

  def initialize(view, version_date)
    @view = view
  ...
  def fetch_kir_wips
    ...
    kir_wips = KirGenotypeWip.where(method_version:@version_date).order("#{sort_column} #{sort_direction}")
